...
import { Users, GraduationCap, UserCog, UserRoundX, MessageCircleMore  } from "lucide-react";
        ...
        {roles?.map((role) => (
            <> 
                <div className="flex mx-40">
                    ...
                    <button
                        className="my-1 justify-center absolute right-45 flex w-40 rounded-sm border border-stroke py-1 text-base outline-none transition-all duration-300 hover:border-primary hover:bg-primary/5 hover:text-primary dark:border-transparent dark:bg-[#2C303B] dark:hover:border-primary dark:hover:bg-primary/5 dark:hover:text-primary dark:hover:shadow-none"
                    >
                        Add Member
                    </button>
                </div>   
                <hr className="my-2 mx-40"/>
                {users?.map((user) => (
                    <>
                        {user.AppUserRoleMappings.map(x => x.RoleId).includes(role.Id) ? (
                            <div className="flex items-center gap-x-4 pb-4 pl-3 my-2 mx-40">
                                ...
                                <div className="flex absolute right-45 gap-2">
                                    <button
                                        className="my-1 justify-center flex w-20 rounded-sm border border-stroke py-1 text-base outline-none transition-all duration-300 hover:border-primary hover:bg-primary/5 hover:text-primary dark:border-transparent dark:bg-[#2C303B] dark:hover:border-primary dark:hover:bg-primary/5 dark:hover:text-primary dark:hover:shadow-none"
                                    >
                                        <Link href="">
